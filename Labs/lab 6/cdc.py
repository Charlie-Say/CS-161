'''
 use csv file and output prompted columns and rows of max and min of death rate.

 The program will read the data from the csv file and find the states with the best
 and worst record for each of these indicators. It will produce a file called best_and_worst.txt
 which lists the states that have the highest and lowest value for each of the indicators,
...
read file
read select columns
find min & max of columns
find index of min & max cells to find state
input data into a dataframe using pandas
'''
...
def main():
    '''
    creating getopt options. open and read the file using pandas.
    find the min & max of selected columns.
    '''
